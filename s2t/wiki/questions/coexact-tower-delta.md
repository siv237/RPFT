...
The tower is not zero in the ordinary Maxwell--ghost/Hodge sector. A direct `RP^3` parity projection was implemented in `s2t_coexact_tower_audit.py` and recorded in `s2t_coexact_tower_results.json`.
...
- `tome2_s2t_spectral_closure.tex` — main text; now includes the coexact tower protocol and Maxwell--ghost--Dirac audit.
- `s2t_coexact_tower_audit.py` and `s2t_coexact_tower_results.json` — `RP^3` coexact tower computation.
...
This was tested in `s2t_pi4_tower_hypothesis_audit.py` with results in `s2t_pi4_tower_hypothesis_results.json`.
...
A follow-up residual audit in `s2t_pi4_residual_audit.py` found a surprisingly strong candidate for the remaining `0.3053%` mismatch. The needed multiplier is:
...
The integer `10` in the Casimir-mixing lead now has a concrete spectral candidate, tested in `s2t_integer10_origin_audit.py` with results in `s2t_integer10_origin_results.json`.
...
The script `s2t_full_coexact_delta_audit.py` computed normalization variants for treating the coexact tower as an independent correction to the current `S_vac` formula.
...
The dedicated audit `s2t_lens_pform_absorption_audit.py` tested the best current route:
...
The audit `s2t_mixed_trace_closure_matrix.py` separates numerical support from theorem requirements.
...
The audit `s2t_c6_operator_trace_skeleton.py` localizes the remaining C6 proof gap. It is no longer useful to search for new nearby integers; the missing work is an explicit second variation of the gauge-fixed Maxwell--ghost determinant under first ambient strain.
...
The audit `s2t_p02_volume_normalization_audit.py` checks whether the prefactor `pi^2/2` is fitted or geometrically forced.
...
The audit `s2t_c6_second_variation_sign_audit.py` checks whether the sign in
...
The audit `s2t_c6_ghost_exact_isolation_audit.py` checks the main caveat left by the sign audit: whether the same `P02` trace-square leaks into the ghost determinant with the opposite Grassmann sign.
...
The audit `s2t_c6_l21_normalization_audit.py` fixes the quotient-normalization convention needed before evaluating the coexact mixed trace.
...
The audit `s2t_c6_l21_shell_selection_audit.py` rules out a too-simple C6 rescue: `P_0,2` shell selection alone does not truncate the coexact tower.
...
The audit `s2t_c6_l21_coexact_locality_gate_audit.py` rules out another over-simple rescue: not every allowed coexact shell channel can be declared local.
...
The audit `s2t_c6_l21_low_shell_block_spec_audit.py` specifies the first finite block that must be computed to decide the coexact absorption route.
...
The audit `s2t_c6_l21_n1_killing_overlap_audit.py` gives the first concrete low-shell warning.
...
The audit `s2t_c6_l21_n1_principal_symbol_audit.py` checks the first actual operator term on the `n=1` Killing shell.
