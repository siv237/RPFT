...
- `dirac_spin_holonomy_results.json` shows coefficient invariance across spin twists and identifies the antiperiodic sector with `theta = pi`.
- `gauge_holonomy_results.json` shows phase-branch motion under `beta` while preserving fitted heat coefficients.
- `sector_attribution_results.json` separates holonomy-phase control from mass/load control with a very large sector-separation ratio.
...
The audit `s2t_neutrino_overlap_first_gate_audit.py` tests the obvious no-fit routes.
...
The second audit `s2t_neutrino_qcycle_geodesic_gram_audit.py` constructs the positive operator rather than identifying it with a holonomy angle.
...
The next audit `s2t_neutrino_qcycle_seesaw_embedding_audit.py` shows that the factor can enter the seesaw matrix without first being declared as a standalone scalar mass.
...
- Source paths: `dirac_spin_holonomy_results.json`, `gauge_holonomy_results.json`, `sector_attribution_results.json`, `tome2_s2t_spectral_closure.tex`, `s2t_tome2_results.json`.