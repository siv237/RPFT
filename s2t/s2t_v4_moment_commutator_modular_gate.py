...
def standardize(matrix):
    ...
    return centered / standard_deviation, float(standard_deviation)
...
for sector, (left_casimir, right_casimir) in casimir_endpoints.items():
    ...
    for power in (4, 6):
        ...
        standardized_moments[power], _ = standardize(sector_moment)
    commutator = (
        standardized_moments[4] @ standardized_moments[6]
        - standardized_moments[6] @ standardized_moments[4]
    ) / (2j)
    standardized_commutator, commutator_scale = standardize(commutator)
    constraint_data[sector] = {
        "z4": standardized_moments[4],
        "z6": standardized_moments[6],
        "commutator": standardized_commutator,
        "commutator_scale": commutator_scale,
    }


def modular_roots(orientation_sign):
    roots = {}
    state_eigenvalues = {}
    for sector, constraints in constraint_data.items():
        modular_hamiltonian, _ = standardize(
            constraints["z4"] + orientation_sign * constraints["commutator"]
        )
        state = expm(-modular_hamiltonian)
    ...
    return roots, state_eigenvalues
...
for orientation_sign in (-1, 1):
    roots, eigenvalues = modular_roots(orientation_sign)
    state_spectra[str(orientation_sign)] = eigenvalues
...
best = branch_rows[0]
best_roots, _ = modular_roots(best["orientation_sign"])
best_data = candidate_data(
    best["placement"], best["messenger_ratio"], best["sign_bits"], best_roots
)
...
output = {
    "gate": "version4_moment_commutator_modular",
    "constraint": "K_s=[Z4_s,Z6_s]/(2i), standardized to unit Hilbert-Schmidt variance",
    "commutator_scales": {
        sector: values["commutator_scale"]
        for sector, values in constraint_data.items()
    },
    ...
    "verdict": "the independent moment commutator activates all mixing planes and improves CP, but the mass-selected branch over-rotates the Cabibbo plane",
}

with open(
    "s2t_v4_moment_commutator_modular_gate_results.json",
    "w",
    encoding="utf-8",
) as handle:
