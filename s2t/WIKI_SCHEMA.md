...
Existing project files such as `*.tex`, `*.json`, `RESEARCH_CATALOG.md`, and `RPFT-main/` are treated as current source material unless explicitly moved by the user.
...
For Obsidian compatibility, use `[[page-name]]` links when linking pages inside `wiki/`. Include source file paths in backticks.
