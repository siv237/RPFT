...
for sector, (left_casimir, right_casimir) in casimir_endpoints.items():
    ...
    normalized_parent[sector] = parent / (np.trace(parent) / 3)


def spectral_function(kind, parent):
    if kind.startswith("power_"):
        return np.linalg.matrix_power(parent, int(kind.split("_")[1]))
    if kind.startswith("inverse_"):
        return np.linalg.matrix_power(
            np.linalg.inv(parent), int(kind.split("_")[1])
        )
    if kind == "exponential_forward":
        return expm(parent - np.eye(3))
    if kind == "exponential_inverse":
        return expm(np.eye(3) - parent)
    if kind == "centered_square":
        centered = parent - np.eye(3)
        return np.eye(3) + centered @ centered
...
function_values = {
    kind: {
        sector: spectral_function(kind, parent)
        for sector, parent in normalized_parent.items()
    }
    for kind in function_menu
}
...
def candidate_data(kind, messenger_ratio, sign_bits):
    identity = np.eye(3)
    result = {}
    for sector, (first_edge, second_edge) in signed_sector_pairs(sign_bits).items():
        ...
        family_function = function_values[kind][sector]
        heavy_block = np.block(
            [
                [
                    messenger_ratio * identity + left_casimir * family_function,
                    1j * identity,
                ],
                [
                    -1j * identity,
                    messenger_ratio * identity + right_casimir * family_function,
                ],
            ]
        )
        ...
        yukawa = (
            projector_odd
            - endpoint_row @ np.linalg.inv(heavy_block) @ endpoint_row.conj().T
        )
...
def mass_objective(log_gap, kind, sign_bits):
    messenger_ratio = 1 + np.exp(float(log_gap))
    data = candidate_data(kind, messenger_ratio, sign_bits)
...
for kind in function_menu:
    ...
    for sign_bits in itertools.product((-1, 1), repeat=4):
        fit = differential_evolution(
            lambda value: mass_objective(value[0], kind, sign_bits),
            [(-10, 15)],
            seed=1729,
            tol=1e-9,
            polish=True,
    ...
    function_rows.append({"function": kind, **sign_rows[0]})

function_rows.sort(key=lambda row: row["mass_log_rms"])
best = function_rows[0]
best_data = candidate_data(
    best["function"], best["messenger_ratio"], best["sign_bits"]
)
...
involution_residuals = {
    str(edge): float(np.linalg.norm(operator @ operator - np.eye(3)))
    for edge, operator in edge_operators.items()
}

output = {
    "gate": "version4_bounded_nonlinear_propagator",
    ...
    "edge_involution_residuals": involution_residuals,
    "asymptotic_formula": "Y_s=P_- - 2/t I + O(t^-2)",
    "verdict": "bounded nonlinear heavy-block deformations cannot split the two light families at leading order",
}

with open(
    "s2t_v4_bounded_nonlinear_propagator_gate_results.json",
    "w",
    encoding="utf-8",
) as handle:
