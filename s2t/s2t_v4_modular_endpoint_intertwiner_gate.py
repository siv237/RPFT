...
for sector, (left_casimir, right_casimir) in casimir_endpoints.items():
    ...
    normalized_parent = parent / (np.trace(parent) / 3)
    state = expm(-normalized_parent)
    ...
    parent_eigenvalues[sector] = np.linalg.eigvalsh(normalized_parent).tolist()
...
output = {
    "gate": "version4_modular_endpoint_intertwiner",
    "gibbs_definition": "rho_s=exp(-Rhat_s)/Tr exp(-Rhat_s)",
    ...
    "leading_metric_left_gns": "L_s L_s^dagger=2 rho_s",
    "bounded_no_go_escaped": True,
    "mass_train_pass": False,
    "ckm_blind_pass": False,
    "verdict": "modular endpoint weighting breaks leading degeneracy and improves the Cabibbo plane but does not close masses or full CKM",
}

with open(
    "s2t_v4_modular_endpoint_intertwiner_gate_results.json",
    "w",
    encoding="utf-8",
) as handle:
