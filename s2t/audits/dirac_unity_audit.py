...
def build_dirac_squared_spectrum(R3: float, R1: float, spin_structure: str = 'periodic'):
    vals = []
    mult = []
    shift = 0.0 if spin_structure == 'periodic' else 0.5
...
def fit_dirac_coefficients(R3: float, R1: float, ts, spin_structure: str = 'periodic'):
    vals, mult = build_dirac_squared_spectrum(R3, R1, spin_structure)
...
def kernel_family_checks(R3: float, R1: float, spin_structure: str = 'periodic'):
    vals, mult = build_dirac_squared_spectrum(R3, R1, spin_structure)
    kernels = [
    ...
    for kernel in kernels:
        ...
        mean = sum(v['volume_normalized'] for v in values) / len(values)
        spread = max(v['volume_normalized'] for v in values) - min(v['volume_normalized'] for v in values)
        rows.append({
            'kernel': kernel,
            'values': values,
            'mean_volume_normalized': mean,
            'spread_volume_normalized': spread,
...
results = {
    'operator': 'Dirac-type squared heat-trace audit on S^3 x S^1',
    'spin_structure_s1': 'periodic',
    'truncation': {'n_max': N_MAX, 'm_max': M_MAX},
    'window_stability': {name: fit_dirac_coefficients(1.0, 1.0, ts) for name, ts in window_checks.items()},
    'radius_sweep': [fit_dirac_coefficients(R3, R1, baseline_window) for R3, R1 in radius_grid],
    'kernel_family': kernel_family_checks(1.0, 1.0),
}

with open('dirac_unity_results.json', 'w', encoding='utf-8') as handle:
