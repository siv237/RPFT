...
- `s2t/docs/tome2_s2t_spectral_closure.tex` — primary S2T / spectral closure source file.
- `s2t/docs/technical_s2t_analysis.tex` — technical companion for the S2T reinterpretation layer.
- `s2t/results/s2t_tome2_results.json` — numerical audit record for the Tome 2 closure checks.
...
- `s2t/results/dirac_spin_holonomy_results.json`, `s2t/results/gauge_holonomy_results.json`, and `s2t/results/sector_attribution_results.json` — audit files to mine for overlap, holonomy, and sector-attribution constraints.
...
- Ingest `s2t/docs/tome2_s2t_spectral_closure.tex` into [[s2t-reinterpretation]] and this roadmap.
- Summarize `s2t/results/s2t_tome2_results.json` in [[numerical-audits]] with pass/fail status and relevant residuals.
...
The main manuscript `s2t/docs/tome2_s2t_spectral_closure.tex` now includes the latest C6/C11 status in its conclusion. It states that `C_conn2[1,1]` has been assembled from product, single-`GammaAB`, and metric-cross tables, is nonzero for all `55` raw symmetric strain pairs, and has rank distribution `2:6`, `4:39`, `6:10`.
