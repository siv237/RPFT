...
- Numerically, `s2t/results/s2t_tome2_results.json` reproduces `S_vac` against `alpha^{-1}` with residual around `-3.48e-9`.
...
- `s2t/docs/tome2_s2t_spectral_closure.tex` now reflects this audit: `S_vac` is a conditional success until `kappa_Cas = 1/24` is closed.
...
- Strict RPFT files checked: `архив-2025-2026/2025-12..2026-01-строгость/rigorous-en/05_pi4_derivation.md`, `архив-2025-2026/2025-12..2026-01-строгость/rigorous-en/08_jacobian_derivation.md`, `архив-2025-2026/2025-12..2026-01-строгость/rigorous-en/15_why_K.md`, `архив-2025-2026/2025-12..2026-01-строгость/rigorous-en/18_pi_term_rigorous.md`, `архив-2025-2026/2025-12..2026-01-строгость/rigorous/30_qed_one_loop_proof.md`, and `архив-2025-2026/2025-12..2026-01-строгость/rigorous/31_geometry_proof.md`.
...
- Source paths: `s2t/docs/tome2_s2t_spectral_closure.tex`, `RPFT-main/rigorous/05_pi4_derivation.md`, `RPFT-main/rigorous/08_jacobian_derivation.md`, `RPFT-main/rigorous/15_why_K.md`, `RPFT-main/rigorous/18_pi_term_rigorous.md`, `RPFT-main/rigorous/30_qed_one_loop_proof.md`, `RPFT-main/rigorous/31_geometry_proof.md`, `s2t/results/s2t_tome2_results.json`.
...
- `s2t/results/s2t_coexact_tower_results.json` gives `T_coex^RP3 = 1.5227161455271536e-05`.
...
The follow-up script `s2t/audits/s2t_determinant_casmix_audit.py` tested the explicit second-order form:
...
A stronger candidate for the missing finite-rank selection rule was added in `s2t/audits/s2t_p02_projector_audit.py`.
...
A conditional derivation was added via `s2t/audits/s2t_metric_variation_p02_audit.py` and inserted into Tome II.
...
The script `s2t/audits/s2t_first_strain_selection_audit.py` formalizes why the `pi^-4` mixed term should use first ambient strain rather than arbitrary internal metric perturbations.
...
`s2t/audits/s2t_full_coexact_delta_audit.py` tested the two `4 -> 5` routes directly.
...
The audit `s2t/audits/s2t_c6_scalar_fp_bookkeeping_audit.py` turns the FP residual warning into a determinant-power check. Bare standard FP gives:
...
The audit `s2t/audits/s2t_c6_scalar_rescue_routes_audit.py` checks whether the standard-FP scalar obstruction can be rescued by locality/subtraction or by zero/gauge/Jacobian cancellation. Both fail as theorem-level claims in the current model.
...
The audit `s2t/audits/s2t_c6_l21_normalization_audit.py` closes a small but important normalization layer before the explicit coexact mixed-trace calculation.
...
The audit `s2t/audits/s2t_c6_l21_shell_selection_audit.py` tests whether the finite rank `10` can be derived from representation-level shell selection alone.
...
The audit `s2t/audits/s2t_c6_l21_coexact_locality_gate_audit.py` tests whether the infinite allowed coexact shell channels can be dismissed as local counterterms.
...
The audit `s2t/audits/s2t_c6_l21_low_shell_block_spec_audit.py` converts the next C6 step into a concrete finite calculation.
...
The audit `s2t/audits/s2t_c6_l21_n1_killing_overlap_audit.py` tests whether the lowest diagonal channel `1 -> 1` can be dismissed by parity or representation symmetry.
...
The audit `s2t/audits/s2t_c6_l21_n1_principal_symbol_audit.py` evaluates the principal-symbol part of the full one-form operator variation on the `n=1` Killing shell in the reduced conformal slice `h=2qg`.
