...
- Spin/holonomy sweep: `s2t/results/dirac_spin_holonomy_results.json` keeps `a0` and `a2` invariant across five twists with max relative errors `2.0624484157763978e-15` and `2.4634800521773637e-13`.
- Gauge doublet sweep: `s2t/results/gauge_holonomy_results.json` moves the phase branches `theta_+/pi: 1.0 → 2.0` and `theta_-/pi: 1.0 → 0.0` while keeping coefficient errors near `10^-13`.
- Sector attribution: `s2t/results/sector_attribution_results.json` assigns phase control to `beta` and subleading spectral load to `mu_heavy`, with separation ratio `1761167242253.6758`.
...
- Source paths: `s2t/results/dirac_unity_results.json`, `s2t/results/dirac_spin_holonomy_results.json`, `s2t/results/gauge_holonomy_results.json`, `s2t/results/enriched_sector_map_results.json`, `s2t/results/sector_attribution_results.json`, `s2t/results/spectral_bridge_results.json`.