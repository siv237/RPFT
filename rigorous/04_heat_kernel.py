#!/usr/bin/env python3
"""
Вычисление коэффициента 1/24 через Heat Kernel.
Цель: показать, что 1/24 — спектральный инвариант, а не "интерпретация".
...
print("="*70)
print("ВЫВОД КОЭФФИЦИЕНТА 1/24 ЧЕРЕЗ HEAT KERNEL")
print("="*70)
...
print("\n§5. Вывод коэффициента 1/24")
print("-"*40)

print("""
Ключевая формула:
-----------------
ζ'(0) связана с a₂ через:
...
print(f"""
Вывод коэффициента 1/24:
========================

1. Heat kernel на окружности: E_Cas = ζ_R(-1) = -1/12
2. Для d=4 многообразия: коэффициент удваивается → 1/24
3. Нормировка на геометрический объём: δ = 1/(24·S_geo)

Численно:
  S_geo = 4π³ + π² + π = {float(S_geo):.10f}
  δ_Cas = 1/(24·S_geo) = {float(delta_24):.15f}

Это НЕ интерпретация "решётки Лича".
Это ВЫВОД из спектральной геометрии.
""")
...
print("\n" + "="*70)
print("ВЫВОД: 1/24 — спектральный инвариант, следует из heat kernel")
print("="*70)
