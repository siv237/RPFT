...
| `wiki/sources/*.md` (204) | Описания источников: тома, программа S2T, каталог, литература, корпуса, дотомовый и полный живой формульные индексы; старый идентификатор Тома I сохранён как перенаправление | [Активно] |
| `wiki/syntheses/*.md` (58) | Сводки и результаты: глобальный формульный атлас, генеалогия, точные и семантические эквивалентности формул, реестр строгих результатов и закрытых путей, систематика томов, дорожные карты, формальная верификация, ведомости статусов, интуитивные карты, археология селектора копий, виртуального цветного моста и точного Hodge--cycle маршрута Тома VII | [Активно] |
| `wiki/questions/*.md` (465) | Гейты и вопросы: по сути индекс всех проверок проекта | [Активно] |
...
Последние и самые свежие данные (активность по 28.08.2026). 1672 исходных
артефакта верхнего уровня, разложенных по типам: 595 `results/*.json`,
573 `audits/*.py`, 446 `gates/*.tex`, 34 `docs/*`,
...
|---|---|---|
| `s2t/gates/` (454) | Гейт-документы `*.tex`: `version3_*` (27), `version4_*` (115), ветви `version5_*`, `version6_*`, `version7_*` и тематические пространства | [Активно] |
| `s2t/audits/` (581) | Вычислительные аудиты `*.py` (включая аудиты Томов VI–VII) | [Активно] |
| `s2t/results/` (603) | Численные результаты `*.json`, парные к аудитам по имени | [Активно] |
| `s2t/docs/` (35) | Тома, интеграционные тексты и файлы сборки; Тома VI–VII завершены как различающие исследовательские тесты | [Активно] |
