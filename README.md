...
| `wiki/sources/*.md` (147) | Описания источников: тома, программа S2T, каталог, литература, корпуса, дотомовый и полный живой формульные индексы; старый идентификатор Тома I сохранён как перенаправление | [Активно] |
| `wiki/syntheses/*.md` (49) | Сводки и результаты: глобальный формульный атлас, генеалогия, точные и семантические эквивалентности формул, реестр строгих результатов и закрытых путей, систематика томов, дорожные карты, формальная верификация, ведомости статусов, аудиты направлений и интуитивная карта межрёберного сцепления Тома VII | [Активно] |
| `wiki/questions/*.md` (429) | Гейты и вопросы: по сути индекс всех проверок проекта | [Активно] |
| `wiki/lints/*.md` (7) | Периодические проверки здоровья вики, включая аудит дедупликации и ссылок | [Справочно] |
...
Последние и самые свежие данные (активность по 26.08.2026). 1552 исходных
артефакта верхнего уровня, разложенных по типам: 555 `results/*.json`,
533 `audits/*.py`, 406 `gates/*.tex`, 34 `docs/*`,
...
|---|---|---|
| `s2t/gates/` (406) | Гейт-документы `*.tex`: `version3_*` (27), `version4_*` (115), ветви `version5_*`, `version6_*`, `version7_*` и тематические пространства | [Активно] |
| `s2t/audits/` (533) | Вычислительные аудиты `*.py` (включая аудиты Томов VI–VII) | [Активно] |
| `s2t/results/` (555) | Численные результаты `*.json`, парные к аудитам по имени | [Активно] |
| `s2t/docs/` (34) | Тома, интеграционные тексты и файлы сборки; Том VI завершён, Том VII исследует рангоизменяющий родитель | [Активно] |
...
| `tome7_s2t_rank_change_parent.tex` | Том VII проверяет рангоизменяющий родитель. Endpoint имеет `U(3)^3`, старый граф не смешивает рёбра; конечный перебор выделил условный минимальный `R2=(3,2)_(7/6)`, который ещё не прошёл Real/первый порядок и цветовой вакуум | [Активно] |
| `version7_introduction_and_problem_statement.tex` | Входной контракт Тома VII: общий носитель, одна суперсвязность, одна норма кривизны и полный физический гессиан до любой карты к частицам | [Активно] |
...
6. Доказанное, условное и закрытые пути Томов I–VII → `wiki/syntheses/global-theorem-and-no-go-ledger.md`
...
10. Термины → `GLOSSARY.md`
11. Историческая база → `архив-2025-2026/2025-12..2026-01-строгость/rigorous/`, `архив-2025-2026/2026-01-дедукция/`
12. Финальные тексты → `corpus/`
