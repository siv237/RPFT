...
| `wiki/sources/*.md` (22) | Описания источников: тома, каталог, литература, корпуса | [Активно] |
| `wiki/syntheses/*.md` (34) | Сводки и результаты: дорожные карты, ведомости статусов, аудиты направлений | [Активно] |
| `wiki/questions/*.md` (304) | Гейты и вопросы: по сути индекс всех проверок проекта | [Активно] |
...
Последние и самые свежие данные (активность по 18.08.2026). 1185 исходных
артефакта верхнего уровня, разложенных по типам: 435 `results/*.json`,
413 `audits/*.py`, 285 `gates/*.tex`, 28 `docs/*` (25 tex + 3 md),
...
|---|---|---|
| `s2t/gates/` (285) | Гейт-документы `*.tex`: `version3_*` (27), `version4_*` (115), новые `version5_*` и тематические (`wilson_*`, `family_*`, `bl_*`, `majorana_*`, `state_menu_*`, `su5_*`, `pati_salam_*`, `external_*`, `c6_*` и др.) | [Активно] |
| `s2t/audits/` (413) | Вычислительные аудиты `*.py` (включая `unified_theory_phase_map.py`) | [Активно] |
| `s2t/results/` (435) | Численные результаты `*.json`, парные к аудитам по имени | [Активно] |
| `s2t/docs/` (28) | Тома и интеграционные тексты; Том V ведётся по-русски, английское написание сохраняется только в именах файлов, обозначениях и библиографии | [Активно] |
...
| `tome5_s2t_parent_architecture.tex` | Том V — класс 15, минимальный дефицит `1/7` и относительный отклик доказаны. Эта/WZW-фаза полной Real-пары сокращается и не выбирает ненулевой сектор; его возможное глобальное принуждение остаётся следующим гейтом | [Активно] |
