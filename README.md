...
| `wiki/questions/*.md` (413) | Гейты и вопросы: по сути индекс всех проверок проекта | [Активно] |
...
Последние и самые свежие данные (активность по 21.08.2026). 1516 исходных
артефактов верхнего уровня, разложенных по типам: 542 `results/*.json`,
520 `audits/*.py`, 393 `gates/*.tex`, 37 `docs/*`,
...
|---|---|---|
| `s2t/gates/` (393) | Гейт-документы `*.tex`: `version3_*` (27), `version4_*` (115), ветви `version5_*`, `version6_*` и тематические пространства | [Активно] |
| `s2t/audits/` (520) | Вычислительные аудиты `*.py` (включая аудиты Тома VI) | [Активно] |
| `s2t/results/` (542) | Численные результаты `*.json`, парные к аудитам по имени | [Активно] |
| `s2t/docs/` (37) | Тома, интеграционные тексты и файлы сборки; Том VI перешёл от механизма к полю порядка и дефектным секторам | [Активно] |
...
| `tome6_s2t_matter_birth.tex` | Том VI — для характерной линии `-1` найден точный пространственный форм-фактор `rho_rad(0)=2*pi` и коэффициент `4*pi^2`, но эффект является секулярным излучением: нежелательный характер не затухает, ядро компакттона разрушается, а ортогональная квадратура остаётся нейтральной. Ветвь идёт к замораживанию как механизм рождения | [Активно] |
